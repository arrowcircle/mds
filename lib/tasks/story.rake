...
namespace :story do
  task :fetch_empty_station => :environment do
    ...
    Story.where("radio is null").where("last_fetched_at < ? or last_fetched_at is null or updated_at > last_fetched_at + interval '1 minute'", 1.day.ago).each do |story|
  ...
  task :fetch_empty_date => :environment do
    ...
    Story.where("date is null").where("last_fetched_at < ? or last_fetched_at is null or updated_at > last_fetched_at + interval '1 minute'", 1.day.ago).each do |story|
      story.parse_info_from_site
    end
  ...
  task :fetch_empty_link => :environment do
    ...
    Story.where("link is null").where("last_fetched_at < ? or last_fetched_at is null or updated_at > last_fetched_at + interval '1 minute'", 1.day.ago).each do |story|
      story.parse_info_from_site
    end
