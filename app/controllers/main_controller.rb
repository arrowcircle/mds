...
class MainController < ApplicationController
  ...
  def unparsed
    @stories = Story.where("(updated_at - interval '1 minute') < last_fetched_at and last_fetched_at is not null and parsed is false and fetcher_comment is not null").page(params[:page])
  end
